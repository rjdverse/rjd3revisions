...
if (! requireNamespace("kableExtra", quietly=T)) {
  knitr::kable(descriptive_statistics[selection,,drop=F], format="html")
  warning("Please install kableExtra: install.packages('kableExtra') to get more visual table output")
}else{
  kableExtra::kable_minimal(knitr::kable(descriptive_statistics[selection,,drop=F], format="html"))
}
```
...
if (! requireNamespace("kableExtra", quietly=T)) {
  knitr::kable(main_results, format="html")
}else{
  format_font <- function(x){
    kableExtra::cell_spec(x,
                          ...
                          bold = ifelse(is.na(x),F,
                                 ifelse(substr(x,1,1) == "S", T, F)))
  }
  nc<-ncol(main_results)
  main_results[,2:nc]<-apply(main_results[,2:nc,drop=F], 2, format_font)
  kableExtra::kable_minimal(knitr::kable(main_results, format="html", escape = F))
}
```
