...
```{r function-table, echo = FALSE, eval = TRUE}
theme_design <- function(x) {
    x <- border_remove(x)
    std_border <- fp_border_default(width = 4, color = "white")
    # x <- fontsize(x, size = 10, part = "all")
    x <- font(x, fontname = "Helvetica Neue", part = "all")
    x <- align(x, align = "center", part = "all")

    x <- bg(x, bg = "gray94", part = "body")
    x <- bg(x, bg = "#7ca2ff", part = "header")
    x <- bg(x, bg = "#7ca2ff", part = "footer")
    x <- color(x, color = "black", part = "all")

    x <- bg(x, j = "Tests", bg = "#7ca2ff")
    x <- color(x, j = "Tests", color = "black")
...
format_column <- function(x, col) {
    x <- bg(x, create_formula(col, "Good"), col, bg = "#4CAF50")
    x <- bg(x, create_formula(col, "Uncertain"), col, bg = "#FFEB3B")
    x <- bg(x, create_formula(col, "Bad"), col, bg = "#FF813A")
    x <- bg(x, create_formula(col, "Severe"), col, bg = "#F44336")
    x <- bold(x, create_formula(col, "Severe"), col)
    x <- color(x, create_formula(col, "Severe"), col, color = "white")
    return(x)
}
...
if (!requireNamespace("flextable", quietly = TRUE)) {
    ...
    warning("Please install flextable: install.packages('flextable') to get more visual table output")
} else {
    library("flextable")
    descriptive_statistics[selection, , drop = FALSE] |>
        format_table() |>
        flextable() |>
        theme_design()
}
...
if (!requireNamespace("flextable", quietly = TRUE)) {
    ...
    main_results |>
        ...
        flextable() |>
        theme_design() |>
        format_column(col = "[Release[2]]-[Release[1]]") |>
        format_column(col = "[Release[3]]-[Release[2]]")
}
```
