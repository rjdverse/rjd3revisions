...
revision_analysis<-function(vintages, 
  ...
  if(transf.log){
    if(length(vt[vt[!is.na(vt)] < 0])>0){
      warning("Logarithm transformation incompatible with negative data. No transformation considered.",call.=F)
      rv<-rv_notrf
      is_log<-F
    }else{
      vt<-log(vt)
  ...
  if(!is.null(ur)){
    ...
    is_stationary<-ifelse(pc_signif_ur>.8,T,F)
  }else{
    ur_rslt<-NULL
    is_stationary<-T
  }
  ...
  if(is_stationary){
    ...
    if(!is.null(coint)){
      ...
      is_cointegrated<-ifelse(pc_signif_coint>.8,T,F)
    }else{
      is_cointegrated<-F
    }
  }
  ...
  nt<-sum(seasonality)
  is_seasonal<-ifelse(nt/ncol(vt)>.8,T,F) 
  if(is_seasonal){
  ...
  if(transf.diff == "auto"){
    ...
    vts<-vtc<-vt_diff
    is_stationary<-is_cointegrated<-F
  }else if(transf.diff == "none"){
    vts<-vtc<-vt
    if(!is_stationary | !is_cointegrated){
      warning("No differentiation considered even though stationarity and/or cointegration might not be present. This can lead to spurious regression.",call.=F)
    }
    is_stationary<-is_cointegrated<-T
  }
  ...
  if(!is.null(U1)){
    ...
    if(!is.null(U2) && !all(is.nan(U2))==T){
      theil_rslt<-rbind(theil_rslt,U2=round(U2,3))
      theil_q<-eval_U(U2)
      U_det<-"U2"
    }else{
      theil_rslt<-rbind(theil_rslt,U2=rep(NA,length(U1)))
      theil_q<-eval_U(U1,U2=F)
      U_det<-"U1"
      warning("Theil U2 could not be calculated. Theil U1 is considered instead in summary().",call.=F)
    }
  }else{
    theil_rslt<-NULL
  ...
  if(!is.null(ta)){
    ...
    t_q<-eval_pvals(ta[,"pvalue"], h0_good=T)
    ta_q<-eval_pvals(ta[,"pvalue.adjusted"], h0_good=T)
  }else{
    ta_rslt<-NULL
  ...
  if(!is.null(sd)){
    ...
    sd_m_q<-eval_pvals(sd[,"intercept.pvalue"], h0_good=T)
    sd_r_q<-eval_pvals(sd[,"slope.pvalue"], h0_good=T)
  ...
  if(!is.null(eff1)){
    ...
    eff1_m_q<-eval_pvals(eff1[,"intercept.pvalue"],h0_good=T)
    eff1_r_q<-eval_pvals(eff1[,"slope.pvalue"],h0_good=T)
  ...
  if(!is.null(eff2)){
    eff2_rslt<-format_reg_output(eff2, is_log, F)
    eff2_m_q<-c("",eval_pvals(eff2[,"intercept.pvalue"],h0_good=T))
    eff2_r_q<-c("",eval_pvals(eff2[,"slope.pvalue"],h0_good=T))
  ...
  if(!is.null(orth1)){
    orth1_rslt<-format_reg_output(orth1,is_log, F)
    orth1_m_q<-c(rep("",nrevs),eval_pvals(orth1[,"intercept.pvalue"],h0_good=T))
    orth1_r_q<-c(rep("",nrevs),eval_pvals(pf(orth1[,"F"], nrevs, orth1[,"N"]-nrevs-1),h0_good=T))
  ...
  if(!is.null(orth2)){
    orth2_rslt<-format_reg_output(orth2, is_log, F)
    orth2_m_q<-c(rep("",ref),eval_pvals(orth2[,"intercept.pvalue"],h0_good=T))
    orth2_r_q<-c(rep("",ref),eval_pvals(orth2[,"slope.pvalue"],h0_good=T))
  ...
  ac_trf_str<-ifelse(ac_trf == "Log", get_info_transformation(T,F), get_info_transformation(F,F))
  pm_test<-try(apply(rv,2,function(x) ljungbox(x[!is.na(x)],k=2)),silent=T) # Ljung-Box up to k
  
  if(!"try-error" %in% class(pm_test)){
    pm_test_mat<-matrix(unlist(pm_test), ncol=2, byrow=T)[,,drop=F]
    ...
    ac_q<-eval_pvals(ac_rslt$estimates_ljungbox[,"p.value"], h0_good=T)
  }else{
    ac_rslt<-NULL
  ...
  seas_trf_str<-ifelse(seas_trf == "Delta-Log 1", get_info_transformation(T,T), get_info_transformation(F,T))
  lb_test<-try(apply(diff(rv),2,function(x) seasonality_qs(x,freq)),silent=T) # Ljung-Box
  fd_test<-try(apply(diff(rv),2,function(x) seasonality_friedman(x,freq)),silent=T)  # Friedman non-parametric test
  
  if(!"try-error" %in% class(lb_test) & !"try-error" %in% class(fd_test) & freq>1){
    seas_rslt<-list(info_transformation=seas_trf_str,
                    estimates_ljungbox=matrix(unlist(lb_test), ncol=2, byrow = T, dimnames = list(colnames(rv), c("value","p.value"))),
                    estimates_friedman=matrix(unlist(fd_test), ncol=2, byrow = T, dimnames = list(colnames(rv), c("value","p.value"))))
    seas_lb_q<-eval_pvals(seas_rslt$estimates_ljungbox[,"p.value"], h0_good=T)
    seas_fd_q<-eval_pvals(seas_rslt$estimates_friedman[,"p.value"], h0_good=T)
  ...
  if(!is.null(sn)){
    ...
    sn_noise_q<-eval_pvals(sn[,"Noise.pvalue"], h0_good=T)
    sn_news_q<-eval_pvals(sn[,"News.pvalue"], h0_good=F)
  }else{
    sn_rslt<-NULL
...
eval_pvals<-function(pval, h0_good = T, residuals = F){
...
format_reg_output<-function(x, is_log, is_diff){
  ...
  n<-nrow(x_df)
  estim<-x_df[1:(n-13),,drop=F]
  
  norm_test<-list(Jarque_Bera_test=x_df[(n-12):(n-9),,drop=F])
  hsk_test<-list(Breusch_Pagan_test=x_df[(n-8):(n-6),,drop=F],White_test=x_df[(n-5):(n-3),,drop=F],ARCH_test=x_df[(n-2):n,,drop=F])
...
regression_diagnostics<-function(reg_output){
  ...
  jb<-eval_pvals(reg_output[,"JarqueBera.pvalue"], residuals=T)
  bp<-eval_pvals(reg_output[,"BreuschPagan.pvalue"], residuals=T)
  wh<-eval_pvals(reg_output[,"White.pvalue"], residuals=T)
  arch<-eval_pvals(reg_output[,"arch.pvalue"], residuals=T)
...
get_vd_rev <- function(vt, gap){
  ...
  rev<-vt[,idx1,drop=F]-vt[,idx0,drop=F]
...
check_seasonality <- function(x){
  ...
  if(frequency(x)>1){
    x_diff<-diff(x)
    lb_pval<-try(seasonality_qs(x_diff,frequency(x))$pvalue, silent=T) # Ljung-Box
    fd_pval<-try(seasonality_friedman(x_diff,frequency(x))$pvalue, silent=T) # Friedman non-parametric test
    
    test_succeeded<-c(!"try-error" %in% class(lb_pval), !"try-error" %in% class(fd_pval))
    if(all(test_succeeded)){
      pvals<-c(lb_pval, fd_pval)
      seasonality<-ifelse(length(pvals[which(pvals<.05)])==2, T, F)
    }else if(any(test_succeeded)){
      if(test_succeeded[1]){
        seasonality<-ifelse(lb_pval<.01,T,F)
      }else if(test_succeeded[2]){
        seasonality<-ifelse(fd_pval<.01,T,F)
      }
    }else{
      seasonality<-FALSE
    }
