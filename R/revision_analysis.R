...
revision_analysis <- function(
    ...
    if (is.null(vintages)) {
        stop("No vintage found!")
    }
    if (view == "diagonal") {
        if (ncol(vintages[["diagonal_view"]]) < (gap + 1)) {
            stop("The number of releases must be >= (1+gap)")
        }
        if (n.releases < (gap + 1)) stop("'n.releases' must be >= (1+gap)")
    } else {
        if (ncol(vintages[["vertical_view"]]) < (gap + 1)) {
            stop("The number of vintages must be >= (1+gap)")
        }
    }
    ...
    if (is_log) {
        rv_notrf <- get_revisions_view(exp(vt), gap)
    } else {
        rv_notrf <- rv
    }
    ...
    if (transf.diff == "auto") {
        ...
        if (!is_stationary || !is_cointegrated) {
            warning(
                "No differentiation considered even though stationarity ",
                "and/or cointegration might not be present. ",
                "This can lead to spurious regression.",
                call. = FALSE
            )
        }
    ...
    theil_infos <- theil_test_evaluator(
        U1,
        U2,
        N = ds["N", ],
        n_test = ncol(rv),
        thr = list(
            u1 = getOption("theil_u1_threshold"),
    ...
    tat_infos <- tat_test_evaluator(
        tat_test,
        is_log,
        n_test = ncol(rv),
        thr = list(
            t = getOption("t_threshold"),
    ...
    eff1_infos <- eff1_test_evaluator(
        eff1_test,
        is_log,
        is_stationary,
        delta_diff,
    ...
    eff2_infos <- eff2_test_evaluator(
        eff2_test,
        is_log,
        n_test = ncol(rv) - 1,
    ...
    orth1_infos <- orth1_test_evaluator(
        orth1_test,
        is_log,
        nrevs,
        ncol_rv = ncol(rv),
    ...
    orth2_infos <- orth2_test_evaluator(
        orth2_test,
        is_log,
        ref,
        ncol_rv = ncol(rv),
    ...
    ac_test <- try(
        apply(rv, 2, function(x) rjd3toolkit::ljungbox(x[!is.na(x)], k = 2)),
        silent = TRUE
    ) # Ljung-Box up to k
    ac_infos <- ac_test_evaluator(
        ac_test,
        is_log,
        cnames = colnames(rv),
        n_test = ncol(rv),
    ...
    lb_test <- try(
        apply(
            X = diff(rv),
            MARGIN = 2,
            FUN = rjd3toolkit::seasonality_qs,
            period = freq
        ),
        silent = TRUE
    ) # Ljung-Box
    fd_test <- try(
        apply(
            X = diff(rv),
            MARGIN = 2,
            FUN = rjd3toolkit::seasonality_friedman,
            period = freq
        ),
        silent = TRUE
    ) # Friedman non-parametric test
    seas_infos <- seas_tests_evaluator(
        lb_test,
        fd_test,
        is_log,
        cnames = colnames(rv),
    ...
    sn_infos <- sn_test_evaluator(
        ...
        n_test = ncol(rv),
        thr = list(
            noise = getOption("signal_noise1_threshold"),
    ...
    # Output
    lbl <- c(
        paste("Relevancy - Theil", theil_infos$U_det),
        "Bias1 T-test",
    ...
    transformation <- c(
        theil_infos$theil_trf,
        rep(tat_infos$tat_trf, 2),
        rep(sd_infos$sd_trf, 2),
        rep(eff1_infos$eff1_trf, 2),
        rep(eff2_infos$eff2_trf, 2),
        rep(orth1_infos$orth1_trf, 2),
        rep(orth2_infos$orth2_trf, 2),
        ac_infos$ac_trf,
        rep(seas_infos$seas_trf, 2),
        rep(sn_infos$sn_trf, 2)
    )
    evals <- rbind(
        theil_infos$theil_q,
        tat_infos$t_q,
        ...
        eff2_infos$eff2_m_q,
        eff2_infos$eff2_r_q,
        orth1_infos$orth1_m_q,
        orth1_infos$orth1_r_q,
        orth2_infos$orth2_m_q,
        orth2_infos$orth2_r_q,
        ac_infos$ac_q,
        seas_infos$seas_lb_q,
        seas_infos$seas_fd_q,
        sn_infos$sn_noise_q,
        sn_infos$sn_news_q
    )
    ...
    diagnostics_table <- list(
        slope_and_drift = sd_infos$sd_diagnostics,
    ...
    output <- list(
        ...
        bias = list(
            t_ta_test = tat_infos$tat_rslt,
            slope_and_drift = sd_infos$sd_rslt
        ),
        efficiency = list(
            efficiency1 = eff1_infos$eff1_rslt,
            efficiency2 = eff2_infos$eff2_rslt
        ),
        orthogonality = list(
            orthogonality1 = orth1_infos$orth1_rslt,
...
get_vintages_view <- function(vintages, transf.log, view, n.releases) {
    ...
    if (transf.log) {
        if (length(vt[vt[!is.na(vt)] < 0]) == 0) {
            ...
            warning(
                "Logarithm transformation incompatible with negative data. No transformation considered.",
                call. = FALSE
            )
            is_log <- FALSE
        }
...
ur_test_intepretor <- function(ur) {
    if (!is.null(ur)) {
        ...
        pc_signif_ur <- length(ur_ADFpvals[ur_ADFpvals < 0.05]) /
            length(ur_ADFpvals)
...
coint_test_interpretor <- function(coint, is_stationary) {
    if (is_stationary) {
        ...
        if (!is.null(coint)) {
            ...
            pc_signif_coint <- length(coint_pvals[coint_pvals < 0.05]) /
                length(coint_pvals)
...
seasonality_test <- function(x) {
    if (stats::frequency(x) > 1) {
        x_diff <- diff(x)
        lb_pval <- try(
            rjd3toolkit::seasonality_qs(x_diff, stats::frequency(x))[[
                "pvalue"
            ]],
            silent = TRUE
        ) # Ljung-Box
        fd_pval <- try(
            rjd3toolkit::seasonality_friedman(x_diff, stats::frequency(x))[[
                "pvalue"
            ]],
            silent = TRUE
        ) # Friedman non-parametric test
        ...
        test_succeeded <- c(
            !inherits(lb_pval, "try-error"),
            !inherits(fd_pval, "try-error")
        )
...
theil_test_evaluator <- function(U1, U2, N, n_test, thr) {
    if (!is.null(U1)) {
        ...
        if (!is.null(U2) && !all(is.nan(U2))) {
            ...
            U_det <- "U1"
            warning(
                "Theil U2 could not be calculated. Theil U1 is considered instead in summary().",
                call. = FALSE
            )
        }
    } else {
...
sd_test_evaluator <- function(
    ...
    if (!is.null(sd)) {
        ...
        sd_diagnostics <- regression_diagnostics(
            sd,
            thr_res_jb,
            thr_res_bp,
            thr_res_white,
    ...
    sd_trf <- if (is_cointegrated && !is_log) {
        "None"
    } else if (is_cointegrated && is_log) {
        "Log"
    } else if (!is_cointegrated && !is_log) {
        paste("Delta", delta_diff)
    } else {
    ...
    return(list(
        ...
        sd_m_q = sd_m_q,
        sd_r_q = sd_r_q,
        sd_diagnostics = sd_diagnostics,
        sd_trf = sd_trf
    ))
}

eff1_test_evaluator <- function(
    eff1,
    is_log,
    is_stationary,
    delta_diff,
    ...
    if (!is.null(eff1)) {
        ...
        eff1_diagnostics <- regression_diagnostics(
            eff1,
            thr_res_jb,
            thr_res_bp,
            thr_res_white,
    ...
    eff1_trf <- if (is_stationary && !is_log) {
        "None"
    } else if (is_stationary && is_log) {
        "Log"
    } else if (!is_stationary && !is_log) {
        paste("Delta", delta_diff)
    } else {
    ...
    return(list(
        ...
        eff1_m_q = eff1_m_q,
        eff1_r_q = eff1_r_q,
        eff1_diagnostics = eff1_diagnostics,
        eff1_trf = eff1_trf
    ))
}

eff2_test_evaluator <- function(
    eff2,
    is_log,
    n_test,
    thr,
    ...
    if (!is.null(eff2)) {
        ...
        eff2_m_q <- c(
            "",
            eval_test(eff2[, "intercept.pvalue"], threshold = thr)
        )
        eff2_r_q <- c("", eval_test(eff2[, "slope.pvalue"], threshold = thr))
        eff2_diagnostics <- regression_diagnostics(
            eff2,
            thr_res_jb,
            thr_res_bp,
            thr_res_white,
    ...
    return(list(
        ...
        eff2_m_q = eff2_m_q,
        eff2_r_q = eff2_r_q,
        eff2_diagnostics = eff2_diagnostics,
        eff2_trf = eff2_trf
    ))
}

orth1_test_evaluator <- function(
    orth1,
    is_log,
    nrevs,
    ncol_rv,
    ...
    if (!is.null(orth1)) {
        ...
        orth1_m_q <- c(
            rep("", nrevs),
            eval_test(orth1[, "intercept.pvalue"], threshold = thr)
        )
        orth1_r_q <- c(
            rep("", nrevs),
            eval_test(
                stats::pf(orth1[, "F"], nrevs, orth1[, "N"] - nrevs - 1),
                threshold = thr
            )
        )
        orth1_diagnostics <- regression_diagnostics(
            orth1,
            thr_res_jb,
            thr_res_bp,
            thr_res_white,
        ...
        if (ncol_rv > nrevs) {
            orth1_m_q <- orth1_r_q <- c(rep("", nrevs), rep(NA, n_test))
        } else {
            orth1_m_q <- orth1_r_q <- rep("", ncol_rv)
        }
    }
    ...
    return(list(
        orth1_rslt = orth1_rslt,
        orth1_m_q = orth1_m_q,
        orth1_r_q = orth1_r_q,
        orth1_diagnostics = orth1_diagnostics,
        orth1_trf = orth1_trf
    ))
}

orth2_test_evaluator <- function(
    orth2,
    is_log,
    ref,
    ncol_rv,
    ...
    if (!is.null(orth2)) {
        ...
        orth2_m_q <- c(
            rep("", ref),
            eval_test(orth2[, "intercept.pvalue"], threshold = thr)
        )
        orth2_r_q <- c(
            rep("", ref),
            eval_test(orth2[, "slope.pvalue"], threshold = thr)
        )
        orth2_diagnostics <- regression_diagnostics(
            orth2,
            thr_res_jb,
            thr_res_bp,
            thr_res_white,
        ...
        if (ncol_rv > ref) {
            orth2_m_q <- orth2_r_q <- c(rep("", ref), rep(NA, n_test))
        } else {
            orth2_m_q <- orth2_r_q <- rep("", ncol_rv)
        }
    }
    ...
    return(list(
        orth2_rslt = orth2_rslt,
        orth2_m_q = orth2_m_q,
        orth2_r_q = orth2_r_q,
        orth2_diagnostics = orth2_diagnostics,
        orth2_trf = orth2_trf
    ))
}
...
ac_test_evaluator <- function(ac, is_log, cnames, n_test, thr) {
    ...
    ac_trf_str <- ifelse(
        ac_trf == "Log",
        get_info_transformation(TRUE, FALSE),
        get_info_transformation(FALSE, FALSE)
    )
    ...
    if (inherits(ac, "try-error")) {
        ...
        ac_rslt <- list(
            ...
            estimates_ljungbox = pm_test_mat
        )
        ac_q <- eval_test(
            ac_rslt$estimates_ljungbox[, "p.value"],
            threshold = thr
        )
    }
...
seas_tests_evaluator <- function(
    ...
    seas_trf_str <- ifelse(
        seas_trf == "Delta-Log 1",
        get_info_transformation(TRUE, TRUE),
        get_info_transformation(FALSE, TRUE)
    )

    if (
        !inherits(lb_test, "try-error") &&
            !inherits(fd_test, "try-error") &&
            freq > 1
    ) {
        seas_rslt <- list(
            info_transformation = seas_trf_str,
            estimates_ljungbox = matrix(
                unlist(lb_test),
                ncol = 2,
                byrow = TRUE,
                dimnames = list(cnames, c("value", "p.value"))
            ),
            estimates_friedman = matrix(
                unlist(fd_test),
                ncol = 2,
                byrow = TRUE,
                dimnames = list(cnames, c("value", "p.value"))
        ...
        seas_lb_q <- eval_test(
            seas_rslt$estimates_ljungbox[, "p.value"],
            threshold = thr
        )
        seas_fd_q <- eval_test(
            seas_rslt$estimates_friedman[, "p.value"],
            threshold = thr
        )
...
sn_test_evaluator <- function(
    ...
    if (!is.null(sn)) {
        sn_rslt <- list(
            info_transformation = get_info_transformation(
                is_log,
                !is_stationary
            ),
    ...
    sn_trf <- if (is_stationary && !is_log) {
        "None"
    } else if (is_stationary && is_log) {
        "Log"
    } else if (!is_stationary && !is_log) {
...
eval_test <- function(val, threshold, ascending = TRUE) {
    if (is.null(threshold)) {
        stop(
            "Some user-defined thresholds are defined as NULL. ",
            "See ?set_thresholds_to_default or ?set_all_thresholds_to_default ",
            "to reset tests thresholds to their default values",
            call. = FALSE
        )
    }
    ...
    if (
        !all(
            tolower(names(threshold)) %in%
                c("good", "uncertain", "bad", "severe")
        )
    ) {
        stop(
            "Possible values for quality assessment are 'good', 'uncertain', 'bad', 'severe'. Please check your options.",
            call. = FALSE
        )
    }
    if (is.unsorted(threshold)) {
        stop(
            "User-defined thresholds must be defined in an ascending order. See vignette for more information.",
            call. = FALSE
        )
    }
    ...
    for (i in seq_len(n)) {
        ...
        quality_fmt <- paste0(
            paste0(
                toupper(substr(quality, 1, 1)),
                substr(quality, 2, nchar(quality))
            ),
            " (",
...
format_reg_output <- function(x, is_log, is_diff) {
    ...
    return(list(
        info_transformation = info_transformation,
        estimates = estim,
        residuals = tests
    ))
}
...
summary.rjd3rev_rslts <- function(object, ...) {
    ...
    cat(
        ...
        nb_revisions,
        "from",
        start(object$revisions),
        "to",
        end(object$revisions),
        "\n\n"
    )
    cat("List of all tests:\n")
    categories <- setdiff(
        names(object),
        c("call", "revisions", "descriptive.statistics", "summary", "view")
    )
...
build_table <- function(
    ...
    if (requireNamespace("flextable", quietly = TRUE)) {
        if (type == "summary") {
            ...
            stat_desc <- x$descriptive.statistics[
                c(
                    "N",
                    "mean revision",
                    "st.dev.",
        ...
        warning(
            "Please install 'flextable': install.packages('flextable') to get more visual output"
        )
        if (type == "summary") {
            return(x$summary)
        } else if (type == "stats-desc") {
