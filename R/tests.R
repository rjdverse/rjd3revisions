...
#' 
theil<-function(vintages.view, gap=1, na.zero=F){
  q<-vintages.view
  if(na.zero) q[is.na(q)]<-0
  jq<-matrix_r2jd(q)
  theil<-try(.jcall("jdplus/revisions/base/r/Utility", "[D", "theil", jq, as.integer(gap)),silent=T)
  if("try-error" %in% class(theil)){
    warning("theil could not be performed",call.=F)
    return(NULL)
  }
  return (theil)
}
...
#' 
theil2<-function(vintages.view, gap=1, na.zero=F){
  q<-vintages.view
  if(na.zero) q[is.na(q)]<-0
  jq<-matrix_r2jd(q)
  theil2<-try(.jcall("jdplus/revisions/base/r/Utility", "[D", "theil2", jq, as.integer(gap)),silent=T)
  if("try-error" %in% class(theil2)){
    warning("theil2 could not be performed",call.=F)
    return(NULL)
  }
  return (theil2)
}
...
#' 
bias<-function(revisions.view, na.zero=F){
  r<-revisions.view
  if(na.zero) r[is.na(r)]<-0
  jrevs<-matrix_r2jd(r)
  jbias<-try(.jcall("jdplus/revisions/base/r/Utility", "Ljdplus/toolkit/base/api/math/matrices/Matrix;", "bias", jrevs),silent=T)
  if("try-error" %in% class(bias)){
    warning("bias could not be performed",call.=F)
    return(NULL)
  }
...
#' 
slope_and_drift<-function(vintages.view, gap=1, na.zero=F){
  q<-vintages.view
  if(na.zero) q[is.na(q)]<-0
  jq<-matrix_r2jd(q)
  jsd<-try(.jcall("jdplus/revisions/base/r/Utility", "Ljdplus/toolkit/base/api/math/matrices/Matrix;", "slopeAndDrift", jq, as.integer(gap)),silent=T)
  if("try-error" %in% class(jsd)){
    warning("Slope and drift could not be performed",call.=F)
    return(NULL)
  }
  ...
  if(all(is.nan(slope_and_drift))){
    warning("slope_and_drift could not be performed",call.=F)
    return(NULL)
  }
